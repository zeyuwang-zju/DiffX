...
class ImageCaptionSaver:
    ...
    def __call__(self, images, real, masked_real, sem, captions, seen):
        ...
        assert images.shape[0] == len(captions) * 4
...
class Trainer:
    def __init__(self, config):
        ...
        if get_rank() == 0:       
            self.image_caption_saver = ImageCaptionSaver(self.name, nrow=self.config.batch_size * 4)
    ...
    def run_one_step(self, batch):
        ...
        grounding_input = self.grounding_tokenizer_input.prepare(batch)  # {"boxes":boxes, "masks":masks, "positive_embeddings":positive_embeddings} / {"sem":sem, "mask":mask}
    ...
    def save_ckpt_and_result(self):
        ...
        if not self.config.disable_inference_in_training:
            ...
            if "boxes" in batch:
                ...
                real_images_with_box_drawing = [] # we save this durining trianing for better visualization
                for i in range(batch_here):
                    real_images_with_box_drawing.append(batch["image_VIS"][i]*0.5 + 0.5)
                    real_images_with_box_drawing.append(batch["image_NIR"][i]*0.5 + 0.5)
                    real_images_with_box_drawing.append(batch["image_SWIR"][i]*0.5 + 0.5)
                    real_images_with_box_drawing.append(batch["image_THERMAL"][i]*0.5 + 0.5)
            ...
            samples = []
            for i in range(batch_here):
                samples.append(dec_VIS[i])
                samples.append(dec_NIR[i])
                samples.append(dec_SWIR[i])
                samples.append(dec_THERMAL[i])
...
if __name__ == "__main__":
    ...
    # CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 train_diffusion4.py  --yaml_file=configs/mcxface.yaml  --DATA_ROOT=./DATA/mcxface/   --batch_size=2   --save_every_iters 1000   --name mcxface
