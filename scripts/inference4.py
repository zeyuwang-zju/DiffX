...
class Inference:
    ...
    def start_inference(self):
        ...
        os.makedirs(os.path.join(self.name, 'dec_VIS'), exist_ok=True) 
        os.makedirs(os.path.join(self.name, 'dec_NIR'), exist_ok=True)
        os.makedirs(os.path.join(self.name, 'dec_SWIR'), exist_ok=True)
        os.makedirs(os.path.join(self.name, 'dec_THERMAL'), exist_ok=True)
        self.model.eval()
        ...
        for batch in tqdm(self.loader_val): # note: iter_idx is not from 0 if resume training
            ...
            batch_here = batch["image_VIS"].shape[0]
            ...
            dec_VIS, dec_NIR, dec_SWIR, dec_THERMAL = autoencoder_wo_wrapper.decode(samples)
            dec_VIS = torch.clamp(dec_VIS, min=-1, max=1).cpu()
            dec_NIR = torch.clamp(dec_NIR, min=-1, max=1).cpu()
            dec_SWIR = torch.clamp(dec_SWIR, min=-1, max=1).cpu()
            dec_THERMAL = torch.clamp(dec_THERMAL, min=-1, max=1).cpu()

            item = batch['item'][0]
            torchvision.utils.save_image( dec_VIS, os.path.join(self.name, 'dec_VIS', f'{item}.jpg'), nrow=1, normalize=True, scale_each=True, range=(-1,1))
            torchvision.utils.save_image( dec_NIR, os.path.join(self.name, 'dec_NIR', f'{item}.jpg'), nrow=1, normalize=True, scale_each=True, range=(-1,1))
            torchvision.utils.save_image( dec_SWIR, os.path.join(self.name, 'dec_SWIR', f'{item}.jpg'), nrow=1, normalize=True, scale_each=True, range=(-1,1))
            torchvision.utils.save_image( dec_THERMAL, os.path.join(self.name, 'dec_THERMAL', f'{item}.jpg'), nrow=1, normalize=True, scale_each=True, range=(-1,1))
            ...
            if "sem" in batch:
                ...
                if num_classes != None:
                    ...
                    sem = batch["sem"][0]
                    torchvision.utils.save_image( sem, os.path.join(self.name, 'sem', f'{item}.jpg'), nrow=1, normalize=True, scale_each=True, range=(-1,1))
...
if __name__ == "__main__":
    ...
    config.train_dataset_names.MCXFaceGrounding.prob_use_caption = 1
    config.train_dataset_names.MCXFaceGrounding.random_crop = False
    config.train_dataset_names.MCXFaceGrounding.random_flip = False
    ...
    # CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.launch --nproc_per_node=1 inference4.py  --yaml_file=configs/mcxface.yaml  --DATA_ROOT=./DATA/mcxface/  --batch_size=1  --name mcxface
