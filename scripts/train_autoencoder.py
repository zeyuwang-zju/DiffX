...
class Trainer_Autoencoder:
    ...
    def run_one_step(self, batch):
        z = self.autoencoder.encode( batch["image_RGB"], batch["image_TIR"] )
        dec_RGB, dec_TIR = self.autoencoder.decode(z)
        
        loss_mse = (torch.nn.functional.mse_loss(dec_RGB, batch["image_RGB"]) + torch.nn.functional.mse_loss(dec_TIR, batch["image_TIR"]))
        loss_lpips = (self.lpips_loss(dec_RGB, batch["image_RGB"]) + self.lpips_loss(dec_TIR, batch["image_TIR"])).mean()
    ...
    def save_ckpt_and_result(self):
        ...
        if not self.config.disable_inference_in_training:
            ...
            torchvision.utils.save_image( torch.cat([batch["image_RGB"], batch["image_TIR"], dec_RGB, dec_TIR], dim=0), save_path, nrow=self.config.batch_size * 2, normalize=True, scale_each=True, range=(-1,1) )
...
if __name__ == "__main__":
    ...
    # CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 train_autoencoder.py  --yaml_file=configs/flir_text.yaml  --DATA_ROOT=./DATA/flir/   --batch_size=2   --save_every_iters 1000   --name flir
    # CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 train_autoencoder.py  --yaml_file=configs/mfnet.yaml  --DATA_ROOT=./DATA/mfnet/   --batch_size=2   --save_every_iters 1000   --name mfnet
    # CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 train_autoencoder.py  --yaml_file=configs/come.yaml  --DATA_ROOT=./DATA/come/   --batch_size=2   --save_every_iters 1000   --name come
    # CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 train_autoencoder.py  --yaml_file=configs/come_sobel.yaml  --DATA_ROOT=./DATA/come/   --batch_size=2   --save_every_iters 1000   --name come_sobel


    # CUDA_VISIBLE_DEVICES=0,1,2,3 nohup python -m torch.distributed.launch --nproc_per_node=4 train_autoencoder.py  --yaml_file=configs/flir_text.yaml  --DATA_ROOT=./DATA/flir/   --batch_size=2   --save_every_iters 5000   --name _20240901_refined_autoencoder &
