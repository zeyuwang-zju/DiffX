...
class SobelDataset(BaseDataset):
    def __init__(self, 
                sod_path,
                prob_use_caption=1,
                image_size=512, 
    ...
    def __getitem__(self, index):
        ...
        label_path = os.path.join(self.sod_path, 'sobel', f'{item}.jpg')

        image_RGB = Image.open(image_RGB_path).convert('RGB')
        image_TIR = Image.open(image_TIR_path).convert('RGB')
        label = Image.open(label_path).convert("RGB") # semantic class index 0,1,2,3,4 in uint8 representation 
        ...
        label = ( self.pil_to_tensor(label).float()/255 - 0.5 ) / 0.5
