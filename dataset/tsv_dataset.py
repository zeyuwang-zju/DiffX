...
class TSVDataset(BaseDataset):
    def __init__(self, 
                ...
                image_size=512, 
                min_box_size=0.01,
                max_boxes_per_data=8,  # GLIGEN在flickr上设置成了30
    ...
    def __getitem__(self, index):
        ...
        for anno in annos:
            x, y, w, h = anno['bbox']  # x_min, y_min, width, height. the original annotation corresponding to the raw image size.
        ...
        # print("id:", out["id"])  # 1531
        # print("image:", out["image"].shape, out["image"])  #  torch.Size([3, 512, 512]), 数值范围：[-1, 1]
        # print("boxes:", boxes.shape, boxes)  # torch.Size([30, 4])
        # print("masks:", masks.shape, masks)  # torch.Size([30])
        # print("image_masks:", image_masks.shape)  # torch.Size([30])
        # print("text_masks:", text_masks.shape)  # torch.Size([30])
        # print("text_embeddings:", text_embeddings, text_embeddings.shape)  # torch.Size([30, 768])
        # print("image_embeddings:", image_embeddings.shape, image_embeddings)  # torch.Size([30, 768]), 如果只做框生图的话是不需要的
        # print(out["caption"])  # A young man with a camera is sitting in a train while grass goes by outside .
        # exit()
