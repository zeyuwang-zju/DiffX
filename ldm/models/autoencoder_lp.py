...
class DUALAutoencoderLP(nn.Module):
    def __init__(self,
                 ddconfig,
                 embed_dim,
                 scale_factor=1,
        ...
        for i in range(self.num_resolutions):
            setattr(self, f'pyr_conv{i}', torch.nn.Conv2d(6, ddconfig["ch"] * ddconfig["ch_mult"][i], 3, 1, 1))
    ...
    def encode(self, image_RGB, image_TIR):
        ...
        for i in range(len(pyramid_RGB)):
            temp_feat = getattr(self, f'pyr_conv{i}')(torch.cat((pyramid_RGB[i], pyramid_TIR[i]), dim=1))
